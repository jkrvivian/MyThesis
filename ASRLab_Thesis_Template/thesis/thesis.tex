...
物聯網 (Internet-of-Things) 連結起不同裝置並自動化許多繁瑣流程，使我們的生活變得更加便捷。而在物聯網場景中，machine-to-machine（M2M）的發展勢不可檔，它允許機器擁有自己的數位資產並開始參與經濟活動，讓裝置間可以共享和交易其資源。裝置間的資料交換是即時且短暫的，往往特定時段內的資料對當下的應用才有意義，而有效靈活的資料流發布/訂閱 (publish/subscribe) 通訊機制便十分符合這種場景。此外，在此模型之上引入訂閱服務的定價模型（如軟體即服務 (Software-as-a-Service)) 可以使訂閱資料產生價值，讓訂閱者得以根據資料使用量計價。另外，這種定價模型讓資料提供者可以更好地管理預算，且資料消費者可以隨興的訂閱和退訂。但與靜態資料集不同的是，動態的資料流提升了資料所有權和身份驗證的重要性和難度，僅採用憑證頒發機構（Certificate Authority）的中心化服務進行身份驗證有淺在的資安風險，會進而威脅到整個系統的信任基礎。因此，一個不需信任單一機構的資料交易基礎建設是必需的，除了讓參與者可以在不信任任何服務或他人的情況下進行交易，還能驗證資料所有權和完整性。此外，考慮到物聯網設備難以同時進行指定工作和交易流程，訂閱流程的自動化是必要的。在本文中，我們利用分散式帳本技術 (Distributed Ledger Techonology)，在發布/訂閱的基礎架構之上構建去中心化且受信任的資料交易平台，除了能有效地提高運作透明度和系統可擴展性，更利用點對點的加密通訊機制允許在分散式帳本儲存、傳輸、存取和驗證資料流，亦透過智能合約 (smart contract) 實現交易自動化。
...
\begin{figure}[h]
    \centering
    \includegraphics[width=5in]{img/launching_product}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=4.5in]{img/mam_vs_e2ee}
